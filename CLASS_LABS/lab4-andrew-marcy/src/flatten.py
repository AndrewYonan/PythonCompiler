...
# dedicated_function_names = ["print",
#                             "eval",
#                             "input",
#                             "int",
#                             "inject_int",
...
def is_eval_input(node):
    if isinstance(node, ast.Call):
        return node.func.id == "eval"

class FlattenAST():

    def __init__(self, prefix_code="temp_", counter=0):

        self.counter = counter
        self.temp_name_prefix_code = prefix_code

    def flatten_subscript(self, node, suite):

        node.slice = self.flatten(node.slice, suite)
        if not is_atomic(node.slice):
            node.slice = self.get_temp_assign_node(node.slice, suite)

        if not is_atomic(node.value):
            node.value = self.flatten(node.value, suite)
            node.value = self.get_temp_assign_node(node.value, suite)
    ...
    def flatten(self, node, suite=None):
        ...
        if isinstance(node, ast.Module):
            ...
            if isinstance(l_val, ast.Subscript):

                node.value = self.get_temp_assign_node(node.value, suite)
                self.flatten_subscript(l_val, suite) 
            ...
            if is_print(node):
                ...
                if not is_atomic(node.args[0]) or is_primitive_literal(node.args[0]):
                    node.args[0] = self.get_temp_assign_node(node.args[0], suite)

            elif not is_eval_input(node):

                for i in range(len(node.args)):
                    node.args[i] = self.flatten(node.args[i], suite)
                    if not is_atomic(node.args[i]):
                        node.args[i] = self.get_temp_assign_node(node.args[i], suite)
            ...
            for i in range(len(node.elts)):
                ...
                node.elts[i] = self.flatten(node.elts[i], suite)

                if not is_atomic(node.elts[i]):    
                    node.elts[i] = self.get_temp_assign_node(node.elts[i], suite)
        
        elif isinstance(node, ast.Dict):

            for i in range(len(node.keys)):

                node.keys[i] = self.flatten(node.keys[i], suite)
                node.values[i] = self.flatten(node.values[i], suite)

                if not is_atomic(node.keys[i]):
                    node.keys[i] = self.get_temp_assign_node(node.keys[i], suite)
            ...
            node = self.flatten_subscript(node, suite)
