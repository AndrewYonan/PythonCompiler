...
class UnParser():
    ...
    def un_parse(self, node, indent_level, no_indent=False):
        ...
        if isinstance(node, ast.Assign):
            return indent + self.un_parse(node.targets[0], indent_level) + " = " + self.un_parse(node.value, indent_level, no_indent=True) + "\n"
        ...
        if isinstance(node, ast.List):
            ...
            for i in range(len(node.elts)):
                un_parse_str += un_parse(node.elts[i])
        ...
        if isinstance(node, ast.Dict):
            ...
            for i in range(len(node.keys)):
                un_parse_str += (un_parse(node.keys[i]) + " : " + un_parse(node.values[i]))
        ...
        if isinstance(node, ast.Subscript):
            ...
            return un_parse(node.value) + f"[{un_parse(node.slice)}]"
