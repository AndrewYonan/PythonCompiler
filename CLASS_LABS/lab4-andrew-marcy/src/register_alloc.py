...
class UInterferenceGraph:
    ...
    def solve_edges(self):
        ...
        IR_instrs = self.liveness_map.IR_instr_list

        for i in range(len(IR_instrs)):

            IR_instr = IR_instrs[i].instruction
            liveness_set_after = sorted(list(self.liveness_map.get_instr_after(i).liveness_before))

            if isinstance(IR_instr, IR_movl) or isinstance(IR_instr, IR_movzbl):
            ...
            elif isinstance(IR_instr, IR_addl):
            ...
            elif isinstance(IR_instr, IR_negl):
            ...
            elif isinstance(IR_instr, IR_call):
            ...
            elif isinstance(IR_instr, IR_sete):

                for v in liveness_set_after:
                    self.add_edge("eax", v)
            
            elif isinstance(IR_instr, IR_setne):
    ...
    def add_edge(self, v1, v2):
        ...
        if is_numeric(v1):
            print(f"Err : trying to add interference between {v1} (a number) and {v2}")
            exit(1)

        if is_numeric(v2):
            print(f"Err : trying to add interference between {v1} and {v2} (a number)")
            exit(1)
...
class RegisterAllocation:
    ...
    def get_var(self, home):
        for var in self.homes:
            if self.get_home(var) == home:
                return var
        print(f"Err : in get_var() home \'{home}\' doesn't have an associated variable")
