...
class BasicBlock:
    ...
    def get_instr_liveness_after(self, i):
        ...
        if i == len(self.IR_instr_list) - 1: # last instruction in basic block
    ...
    def all_successors_converged(self):
        for successor in self.successors:
            if not successor.liveness_converged:
                return False
        return True
...
class BasicBlockList:
    ...
    def take_updated_instructions_from_cfg(self, cfg):
        self.basic_blocks = []
...
class CFGraph:
    ...
    def set_all_basic_blocks_visited_false(self):
        for block in self.basic_blocks:
            block.visits = 0
            
    def get_liveness_start(self):
        return self.basic_blocks[0].live_in
    ...
    def solve_liveness(self):
        ...
        while 1:
            ...
            self.set_all_basic_blocks_visited_false()
            
            changed_liveness = self.solve(last_block, None, set())
            
            if changed_liveness == 0:
                break
            
            first_block_liveness_in = self.get_liveness_start()
            if len(first_block_liveness_in) > 0:
                print(f"Liveness error : initial liveness_set {first_block_liveness_in} NOT empty")
                exit(1)
    ...
    def solve(self, block, successor, live_in_successor):
        ...
        for predecessor in block.predecessors:
            ...
            # whether live_in of block contributes anything new to live_out of predecessor
            new_predecessor_liveness_contribution = ((block.live_in | predecessor.live_out) != predecessor.live_out)

            if predecessor.visits == 0:

                if not new_predecessor_liveness_contribution: 
                    liveness_changed += self.solve(predecessor, block, block.live_in)

                else:

                    live_out_pred = copy_set(block.live_in)
                    liveness_changed += (1 + self.solve(predecessor, block, live_out_pred))
            
            elif new_predecessor_liveness_contribution:
